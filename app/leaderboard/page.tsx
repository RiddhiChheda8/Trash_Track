  ...
  useEffect(() => {
    const fetchRewardsAndUser = async () => {
      ...
      try {
        ...
        // Filter to show only unique users with their highest points
        const userMap = new Map<number, Reward>()
        
        fetchedRewards.forEach(reward => {
          const existingReward = userMap.get(reward.userId)
          
          // If user not in map OR current reward has higher points, add/update it
          if (!existingReward || reward.points > existingReward.points) {
            userMap.set(reward.userId, reward)
          }
        })
        
        // Convert map to array and sort by points (descending)
        const uniqueRewards = Array.from(userMap.values())
          .sort((a, b) => b.points - a.points)
          .slice(0, 50) // Limit to top 50 performers
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Level</th>
                  </tr>
                </thead>
                <tbody>
                  {rewards.length === 0 ? (
                    ...
                    rewards.map((reward, index) => (
                      <tr 
                        key={`${reward.userId}-${index}`} 
                        className={`
                          ${user && user.id === reward.userId ? 'bg-indigo-50 border-l-4 border-indigo-500' : ''} 
